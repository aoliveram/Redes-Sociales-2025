...
##   Polnet 2023 Workshop                                 ##
##   www.kateto.net/polnet2023                            ##
##                                                        ##
...
# Download the workshop materials: bit.ly/polnet-2023
# Online tutorial: kateto.net/polnet2023
...
# Download an archive with the data files from http://bit.ly/polnet-2023
...
# For example, if the folder was called "polnet2023":
setwd("C:/polnet2023") 
...
as_edgelist(net, names=T)                # ~ links [class(as_edgelist(net, names=T)) == "matrix" "array" ]
as_adjacency_matrix(net, attr="weight")  # ~ nodes

# Or data frames describing nodes and edges:
as_data_frame(net, what="edges")         # ~ [class(as_data_frame(net, what="edges")) == data.frame]
...
#graph_attr(net, "layout") <- layout_with_lgl ## Something is wrong with layout_with_lgl
plot(net) 
...
l <- layout_with_fr(net.bg) # default niter=500
plot(net.bg, layout=l)
?layout_with_fr
