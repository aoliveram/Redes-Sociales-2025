...
red4 <- set_vertex_attr(red4, "letras",   value = V(red4)$V2)
red4 <- set_vertex_attr(red4, "contar_a", value = V(red4)$V3)

V(red4)$letras <- as.integer(V(red4)$letras)
V(red4)$contar_a <- as.integer(V(red4)$contar_a)

red4 <- delete_vertex_attr(red4, "V2")
red4 <- delete_vertex_attr(red4, "V3")

V(red4)$shape <- ifelse(V(red4)$contar_a == 2, "square", "circle")

plot(red4)

plot(red4,
     vertex.size  = 10 + 3*V(red4)$letras,
     vertex.label = V(red4)$name,
     vertex.shape = V(red4)$shape)
...
V(red2)$color <- c("blue","red","red","yellow","pink")
E(red2)$color <- c("blue","red","red","red")
...
# podemos crear una red con pesos 
red2_f <- red2
E(red2_f)$weight <- c(0.1,0.2,1,1.3)
is_weighted(red2_f)
plot(red2_f)
plot(red2_f, edge.width =15*E(red2_f)$weight)
...
l.circulo <- layout_in_circle(red2) # podemos hacer circulos previos y luego ponerlos en el plot
plot(red2, edge.arrow.size=.2,vertex.label=NA, vertex.color=color_nodo,
     vertex.frame.color="black",vertex.shape="circle", edge.curved=.2,
     edge.color="green", vertex.size=size_nodo*10,
     layout=l.circulo)

# y con el layout generado hacer modificaciones que no cambien la disposición de los nodos
plot(red2, edge.arrow.size=.2,vertex.label=NA, vertex.color=color_nodo,
     vertex.frame.color="black",vertex.shape="circle", edge.curved=.3,
     edge.color="green", vertex.size=size_nodo*5,
     layout=l.circulo)
...
red_b_proyecciones$proj1  # proyeccion en palabras
red_b_proyecciones$proj2  # proyección en siglas
...
adj_matrix_completa <- as_adjacency_matrix(red_completa)
print(adj_matrix_completa)

# red estrella
red_estrella <- make_star(10, mode="undirected")
plot(red_estrella)

adj_matrix_estrella <- as_adjacency_matrix(red_estrella)
print(adj_matrix_estrella)
...
adj_matrix_anillo <- as_adjacency_matrix(red_anillo)
print(adj_matrix_anillo)

red_regular <- sample_k_regular(10, k=5, directed = FALSE, multiple = FALSE)
plot(red_regular)

adj_matrix_regular <- as_adjacency_matrix(red_regular)
print(adj_matrix_regular)

# redes árboles 
red_arbol <- make_tree(10, mode = "out")
plot(red_arbol, edge.arrow.size=0.5)

adj_matrix_arbol <- as_adjacency_matrix(red_arbol)
print(adj_matrix_arbol)
...
# analiza la ego network del nodo especificado (por defecto: dirigidas)
neighbors(red2,2)
neighbors(red2,2, mode="in")

neighbors(red2,1, mode="all")

# grado de la red  (NOTA: require(sna) y ver qué pasa) | detach("package:sna", unload = TRUE)
igraph::degree(red2)
igraph::degree(red2, mode="in")
igraph::degree(red2, mode="out")
hist(igraph::degree(red2))
# cuando es una red con pesos se usa strength(_nombre_de_la_red_)
...
# cliques 
# Estos algoritmos son lentos porque son censos de toda la red. Usemos una red más pequeña
data(karate)
plot(karate)
...
reciprocity(red2) # proporción de conexiones bidireccionales sobre el total de conexiones
